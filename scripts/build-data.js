...
const csvFilename = 'school-leaver-destinations-followup.csv';
...
function interrogateSPARQL(zones) {
    ...
    _.forEach(_.chunk(zones, chunk_size), (zone_chunk) => {
        queries.push(`
            ...
            SELECT ?year ?zone ?group ?destination ?measure ?value
            WHERE {
                ?s qb:dataSet data:school-leaver-destinations-followup ;
                   qb:measureType ?m ;
                   ?m ?value ;
                   dim:populationGroup ?g ;
